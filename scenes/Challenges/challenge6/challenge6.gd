...
func _ready():
	...
	for rule in Global.challengeData.get("active_rules", []):
		...
		if rule == "ota":
			var ota: Ota = get_node_or_null("OtaPath/OtaPathFollow/Ota")
			if ota:
				$ChallengeLogic.ota_start_following_uto()
				ota.global_position = Global.challengeData["otaPosition"]
	# remove mockup node
...
func update_paths_to_not_cross_barriers():
	...
	# guards
#		if poet_top_y - half_pawn_size <= lowest_rule_y_pos:
#			poet_top_y = lowest_rule_y_pos + half_pawn_size
...
func puzzle_enable_victory():
	update_guards_path_to_not_overlap_text()
